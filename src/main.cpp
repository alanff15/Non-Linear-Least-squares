...
// data table, vector of data-type
// example: pair (y,x)
...
// error function, returns error, receives params and a line of data as consts
// example: y = f(p,x) | err = (y-f(p,x))²
double err(const double p[], const std::array<double, 2> yx) {
  double ret = yx[0];                                 // y
  ret -= p[0] * exp(-pow((yx[1] - p[1]) * p[2], 2));  // f(p,x)
  return ret * ret;                                   // err
}

// kernel to reject outliers, used as weight for each data line (the smaller the error the bigger the weight)
// example: exponential
...
int main() {
  NonLinearLSQ<double, std::array<double, 2>, 3> lsq;  // create solver <precision, data-type, number of params to find>
  lsq.setData(data);                                   // define data (vector of data-type)
  lsq.setErrorFunction(err);                           // define error function (function tha returns error, receives params and a line of data as consts)

  // optional configs (converge faster / avoid divergency)
  lsq.setKernel(kernel);     // define kernel
  lsq.setLimits(0, 1, 4);    // constraints to param 0
  lsq.setLimits(1, 3, 6);    // constraints to param 1
  lsq.setLimits(2, 0.5, 2);  // constraints to param 2
